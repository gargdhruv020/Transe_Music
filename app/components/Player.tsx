...
export default function Player() {
  ...
  const [queueMode, setQueueMode] = useState<"all" | "16d" | "global" | "goa" | "remix">("all");
  ...
  useEffect(() => {
    ...
    const activeQueue = queueMode === "16d" ? tracks.filter(t => t.isSpatial) : queueMode === "global" ? tracks.filter(t => t.isGlobal) : queueMode === "goa" ? tracks.filter(t => t.isGoa) : queueMode === "remix" ? tracks.filter(t => t.isRemix) : tracks;
  ...
  const handleTrackSelect = useCallback((index: number, mode: "all" | "16d" | "global" | "goa" | "remix") => {
